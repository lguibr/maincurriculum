...
export function handleLanggraphEvent(payload: any, state: AppState) {
  ...
  if (payload.event === "on_tool_end" && payload.name === "fetch_github_repos") {
    try {
      ...
      for (const name of newTargetRepos) {
        newReposProgress[name] = {
          phase: "Pending Initialization...",
          progress: 0,
  ...
  if (payload.event === "on_tool_start") {
    ...
    if (nodeName) {
      ...
      if (
        input &&
        (nodeName === "process_repo" || nodeName === "clone_repo" || nodeName === "embed_project")
      ) {
  ...
  if (payload.event === "on_chain_start") {
    ...
    if (
      nodeName &&
      !["LangGraph", "__start__", "RunnableLambda", "Supervisor", "deep_agent"].includes(nodeName)
    ) {
      newSubagents[runId] = {
        id: runId,
        name: nodeName,
  ...
  if (payload.event === "on_chain_end") {
    ...
    if (nodeName && newSubagents[runId]) {
      ...
      if (payload.data?.output) {
        try {
          const outStr =
            typeof payload.data.output === "string"
              ? payload.data.output
  ...
  if (payload.event === "on_chat_model_stream") {
    ...
    if (nodeName && nodeName !== "Supervisor") {
      if (!newSubagents[runId])
        newSubagents[runId] = { id: runId, name: nodeName, status: "running", content: "" };
...
export function handleLogEvent(msg: string, state: AppState) {
  ...
  if (msg.includes("Found") && msg.includes("target repositories")) {
    ...
    phase = "Extracting Relational Entities...";
    pgr = 95;
  } // Catches the long LLM processing post-repo mapping
  else if (msg.includes("Entities extraction failed")) {
    phase = "Extraction Finished (With Error)";
    pgr = 100;
  }
