...
export const useStore = create<AppState>((set, get) => ({
  ...
  startAgent: async (selectedRepos?: any[]) => {
    ...
    try {
      ...
      await Promise.all([Gemma4Inference.initialize(), GemmaEmbeddings.initialize()]);

      set((s) => ({
        progress: 20,
        logs: [...s.logs, "Models Load successfully. Begin CV Parsing"],
      }));
      ...
      if (selectedRepos && selectedRepos.length > 0) {
        set((s) => ({
          currentPhase: "Embedding Repositories",
          logs: [...s.logs, "Fetching Repository Text from Github..."],
        }));
      ...
      set((s) => ({
        logs: [...s.logs, "Ingestion Pipeline Complete!"],
        progress: 100,
        isRunning: false,
