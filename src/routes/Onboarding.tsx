...
export default function Onboarding() {
  ...
  useEffect(() => {
    ...
    if (wizardPhase === 2 && !store.isRunning && store.currentPhase === "Complete") {
         setWizardPhase(3);
    }
  ...
  useEffect(() => {
    ...
    import("../db/indexedDB").then(({ dbOps }) => {
       dbOps.getProfile("main").then(prof => {
          if (prof) {
             store.setGithubUsername(prof.github_handle || "");
             if (prof.base_cv && prof.base_cv.trim().length > 50) {
                store.setBaseCv(prof.base_cv);
                store.setIsWizardComplete(true);
  ...
  const handleHardReset = async () => {
     if (!confirm("WARNING: This will completely nuke your local database, tokens, and restart the onboarding flow. Proceed?")) return;
     localStorage.clear();
     try {
  ...
  const handleFetchRepos = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://api.github.com/users/${handle}/repos?type=owner&sort=updated&per_page=100`, { headers });
  ...
  const handleSubmitCV = async () => {
    setWizardPhase(4);
    try {
       await store.processCvAndInterview(store.baseCv);
    } catch(e) {
      console.error(e);
    }
  };
      ...
      <main className="flex-1 overflow-hidden p-6 flex flex-col items-center">
        <div className={`w-full flex-1 flex flex-col gap-6 min-h-0 transition-all duration-500 ${wizardPhase === 4 ? 'max-w-[1600px]' : 'max-w-4xl'}`}>
          ...
          <div className="w-full flex-1 flex flex-col bg-card border border-border/50 rounded-2xl overflow-hidden shadow-2xl min-h-0">
            <div className={`p-5 border-b border-border/50 bg-muted/40 shrink-0 ${wizardPhase === 1 ? 'hidden' : ''}`}>
                ...
                <div className="h-full flex flex-col overflow-hidden bg-[#09090b] text-[#f1f3fc] relative rounded-b-2xl">
                  ...
                  {fetchedRepos.length === 0 ? (
                    <div className="relative p-10 flex flex-col items-center justify-center h-full max-w-2xl mx-auto w-full text-center">
                      <img src="/logo.png" alt="Logo" className="h-24 w-auto object-contain mb-10 drop-shadow-[0_0_15px_rgba(255,255,255,0.1)]" />
                      <h1 className="text-3xl font-bold tracking-tight mb-3 text-white">Repository Ingestion</h1>
                      <p className="text-[#a1a1aa] font-mono text-sm tracking-wide mb-10">Enter a GitHub handle to fetch projects for the AI embedding sequence.</p>
                      ...
                      <div className="flex flex-col w-full items-center gap-3">
                        ...
                        <div className="flex flex-col w-full gap-4 mt-6">
                           <div className="flex items-center gap-4 bg-[#18181b] p-4 rounded-xl border border-[#0070eb]/20 animate-in fade-in slide-in-from-top-2">
                             <Label className="text-[#a1a1aa] font-mono text-xs uppercase tracking-widest shrink-0">Cloud Tier:</Label>
                             <div className="flex bg-[#09090b] rounded-lg p-1 border border-[#27272a]">
                               <button 
                                 onClick={() => store.setCloudTier("smart")}
                                 className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${store.cloudTier === 'smart' ? 'bg-[#0070eb] text-white' : 'text-[#52525b] hover:text-[#a1a1aa]'}`}
                               >
                                 Smart
                               </button>
                               <button 
                                 onClick={() => store.setCloudTier("balanced")}
                                 className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${store.cloudTier === 'balanced' ? 'bg-[#0070eb] text-white' : 'text-[#52525b] hover:text-[#a1a1aa]'}`}
                               >
                                 Balanced
                               </button>
                               <button 
                                 onClick={() => store.setCloudTier("widely")}
                                 className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${store.cloudTier === 'widely' ? 'bg-[#0070eb] text-white' : 'text-[#52525b] hover:text-[#a1a1aa]'}`}
                               >
                                 Widely
                               </button>
                             </div>
                             <div className="text-[10px] text-[#52525b] ml-2 hidden sm:block">
                               {store.cloudTier === 'smart' && "Pro (Curriculum) + Flash (Extraction)"}
                               {store.cloudTier === 'balanced' && "Flash (Curriculum) + Lite (Extraction)"}
                               {store.cloudTier === 'widely' && "Pro (Curriculum) + Lite (Extraction)"}
                             </div>
                           </div>
                    ...
                    <div className="relative p-8 flex flex-col overflow-hidden h-full max-w-6xl mx-auto w-full">
                      <div className="flex-1 flex flex-col min-h-0 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex justify-between items-end mb-4 border-b border-[#27272a] pb-4">
                          ...
                          <div className="flex gap-2">
                            <Button variant="outline" size="sm" className="h-9 border-[#27272a] bg-[#18181b] text-[#d4d4d8] hover:text-white" onClick={() => setSelectedRepoUrls(fetchedRepos.map(r => r.url))}>
                              Select All
                            </Button>
                            <Button variant="outline" size="sm" className="h-9 border-[#27272a] bg-[#18181b] text-[#d4d4d8] hover:text-white" onClick={() => setSelectedRepoUrls([])}>
                              Unselect All
                            </Button>
                          </div>
                        </div>
                        <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                          {fetchedRepos.map(repo => (
                            <div key={repo.url} className={`flex items-start space-x-4 p-4 rounded-xl border transition-all cursor-pointer ${selectedRepoUrls.includes(repo.url) ? 'bg-[#18181b] border-white/20' : 'bg-[#09090b] border-[#27272a] hover:border-[#3f3f46]'}`} onClick={() => {
                                 if (selectedRepoUrls.includes(repo.url)) setSelectedRepoUrls(selectedRepoUrls.filter(u => u !== repo.url));
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {repo.description && <p className="text-[#a1a1aa] text-sm mt-1.5 line-clamp-2">{repo.description}</p>}
                ...
                <div className="flex-1 flex min-h-0 bg-background/50 overflow-hidden relative rounded-b-2xl">
                  ...
                  <div className="flex-1 p-6 md:p-10 flex flex-col items-center overflow-y-auto custom-scrollbar bg-[#090e16]">
                    <div className="shrink-0 flex flex-col items-center mt-4">
                      ...
                      <h3 className="text-xl font-bold font-mono tracking-widest text-[#dee2ee] mb-2 uppercase">System Running</h3>
                      <p className="text-[#b9cac9] max-w-sm mb-12 font-mono text-xs text-center">{store.currentPhase || "Initializing Pipeline..."}</p>
                    </div>
