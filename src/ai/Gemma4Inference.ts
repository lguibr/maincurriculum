...
export class Gemma4Inference {
  ...
  static async initialize(modelSize: GemmaModelSize = "e4b", onProgress?: (progress: number) => void): Promise<LlmInference> {
    ...
    try {
      ...
      const modelPath = modelSize === "26b-moe" 
      ...
      console.log(`Loading Gemma 4 (${modelSize === "26b-moe" ? "26B MoE" : "Effective 4B"}) via WebGPU...`);
  ...
  static async generate(prompt: string, format: "json" | "text" = "text", modelSize: GemmaModelSize = "e4b"): Promise<string> {
    const llm = await this.initialize(modelSize);
    // Forcing strict JSON output behavior in prompt if asked.
    const strictPrompt = format === "json" 
