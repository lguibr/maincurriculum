...
describe("Suite D: Frontend React Router DOM / Tailor", () => {
    beforeEach(() => {
        ...
        (global.fetch as any).mockImplementation((url: string) => {
            ...
            if (url.includes("/api/tailor")) {
                return Promise.resolve({
                    json: () => Promise.resolve({ tailoredCv: "Mocked CV", coverLetter: "Mocked Letter", employerAnswers: "Mocked QA" })
                });
            }
    ...
    it("D5. Fire Generate CTA toggles loading states", async () => {
        ...
        await waitFor(() => expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("/api/profile/latest")));
    ...
    it("D8. Completing fetch renders dynamic JSON outputs", async () => {
        render(<Tailor />);
        fireEvent.change(screen.getByPlaceholderText(/Paste the target job description here/), { target: { value: "Role" } });
        await waitFor(() => expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("/api/profile/latest")));
