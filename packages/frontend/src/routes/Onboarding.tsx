...
export default function Onboarding() {
  ...
  const handleFetchRepos = async () => {
    ...
    try {
      ...
      try {
        const profileRes = await fetch(`http://${window.location.hostname}:3001/api/profile/latest`);
                    ...
                    <div className="relative p-8 flex flex-col overflow-hidden h-full max-w-6xl mx-auto w-full">
                      <div className="flex-1 flex flex-col min-h-0 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="pt-6 shrink-0">
                          <Button onClick={handleStartIngestion} disabled={selectedRepoUrls.length === 0} className="w-full h-14 bg-white text-black hover:bg-gray-200 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)]">
