...
import { User, CheckCircle, Send, Loader2, Activity, Play, FileCode, AlertCircle, RefreshCw, Copy, Check, ArrowLeft } from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans selection:bg-primary/30 h-screen overflow-hidden text-sm">
      <header className="shrink-0 flex items-center justify-between p-4 bg-muted/20 border-b border-border/40 backdrop-blur-md">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold tracking-tight">MainCurriculum <Badge variant="secondary" className="ml-2 font-normal">Command Center 2.0</Badge></h1>
        </div>
        ...
        <div className="w-full max-w-7xl flex-1 flex flex-col lg:flex-row gap-6 min-h-0">
          ...
          <div className="lg:w-1/2 w-full flex flex-col bg-card border border-border/50 rounded-2xl overflow-hidden shadow-2xl h-full">
            <div className="p-5 border-b border-border/50 bg-muted/40 shrink-0">
               <h2 className="font-semibold text-base flex items-center"><User className="w-5 h-5 mr-3 text-primary"/> Ingestion Payload</h2>
            </div>
            
            <div className="flex-1 flex flex-col overflow-hidden min-h-0">
               <div className="p-5 border-b border-border/40 bg-background/50 shrink-0 space-y-2">
                 <Label className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">GitHub User / URL</Label>
                 <Input 
               ...
               <div className="flex-1 relative flex flex-col min-h-0 bg-[#1e1e1e]">
                 <div className="absolute top-0 w-full z-10 flex justify-between items-center px-4 py-2 bg-[#2d2d2d] border-b border-[#3c3c3c] shadow-lg">
                   <Label className="text-[10px] text-gray-400 font-mono tracking-widest flex items-center"><FileCode className="w-3 h-3 mr-2"/> BASE_CV.md</Label>
                 </div>
               ...
               <div className="p-4 bg-muted/10 shrink-0 border-t border-border/50">
                 ...
                 >
                   {store.isRunning ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : <Play className="w-5 h-5 mr-2" />}
          ...
          <div className="lg:w-1/2 w-full flex flex-col bg-card border border-border/50 rounded-2xl overflow-hidden shadow-xl h-full min-h-0 relative">
             ...
             <div className="p-6 bg-muted/30 shrink-0 border-b border-border/40">
               <h2 className="text-xl font-bold mb-6 flex items-center"><Activity className="w-6 h-6 mr-3 text-primary"/> Agentic Onboarding</h2>
               <div className="flex flex-col gap-4">
                 {phases.map((step, i) => {
                    const isPassed = phases.findIndex(p => store.currentPhase.includes(p.match)) > i;
                    const isActive = store.currentPhase.includes(step.match);
                    const colorState = isPassed || store.isWizardComplete ? "text-emerald-500" : (isActive ? "text-primary" : "text-muted-foreground");
                    const circleState = isPassed || store.isWizardComplete ? "bg-emerald-500/20 border-emerald-500" : (isActive ? "bg-primary/20 border-primary shadow-[0_0_15px_rgba(var(--primary),0.5)]" : "bg-muted border-border/50");

                    return (
                      <div key={i} className={`flex items-center text-sm font-semibold transition-all ${colorState}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-4 border-2 ${circleState}`}>
                          {isPassed || store.isWizardComplete ? <CheckCircle className="w-4 h-4" /> : i + 1}
                        </div>
                        {step.name}
                    ...
                    <div className="w-full relative shadow-2xl rounded-2xl mb-8 border border-primary/30 overflow-hidden flex flex-col max-h-[60vh]">
                      ...
                      <div className="relative p-6 bg-card flex flex-col flex-1 overflow-y-auto custom-scrollbar">
                         <div className="flex justify-between items-start mb-4 shrink-0">
                           <Label className="text-xs text-primary tracking-widest font-bold uppercase mt-1">{store.currentPhase} Question</Label>
                           <Button 
                             variant="ghost" 
                             size="icon" 
