...
export default function Dashboard() {
  ...
  const isActive = (path: string) => location.pathname === path || (path === '/tailor' && location.pathname === '/');
    ...
    <div className="h-screen w-screen bg-background text-foreground flex font-sans overflow-hidden print:h-auto print:overflow-visible print:block print:bg-white print:text-black">
      ...
      <div className="w-64 border-r border-border/40 bg-card/50 flex flex-col shrink-0 print:hidden">
         ...
         <nav className="flex-1 p-4 flex flex-col gap-2">
            <div className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Agent Apps</div>
            ...
            <div className="mt-6 mb-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Vector Memory</div>
            ...
            <div className="mt-4 mb-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">System</div>
            <Link 
              to="/onboarding"
            ...
            <div className="mt-auto pt-4 border-t border-border/40 mb-2 text-xs font-semibold text-red-500 uppercase tracking-wider">Danger Zone</div>
            <button 
              onClick={async () => {
               if (window.confirm("WARNING: This will permanently wipe all agents, CV versions, and ingested vector memory! Proceed?")) {
                   await fetch(`http://${window.location.hostname}:3001/api/reset`, { method: "DELETE" });
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden print:overflow-visible print:block print:bg-white">
        ...
        <header className="h-14 border-b border-border/40 bg-card flex items-center px-4 shrink-0 justify-between print:hidden">
          <div className="flex items-center text-sm text-muted-foreground font-medium">
             ...
             <Link to={location.pathname} className="text-primary font-semibold capitalize hover:opacity-80 transition-opacity">
