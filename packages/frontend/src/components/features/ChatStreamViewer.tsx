  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
      {visibleStreams.map(([nodeName, text]) => {
        ...
        return (
          <Card key={nodeName} className={`bg-neutral-900 border-neutral-800 shadow-lg transition-transform ${isActive ? 'ring-1 ring-emerald-500/50' : ''}`}>
            ...
            <CardContent className="p-0">
              <ScrollArea className="h-48 p-4">
                <div className="text-xs text-neutral-300 whitespace-pre-wrap font-mono leading-relaxed">
                  {text}
                  {isActive && <span className="inline-block w-1.5 h-3.5 bg-emerald-500 ml-1 animate-pulse" />}
                </div>
              </ScrollArea>
            </CardContent>
          </Card>
