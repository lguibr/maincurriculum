...
async function evaluateCompleteness(state: typeof StateAnnotation.State, config?: RunnableConfig) {
    ...
    if (state.userProfileId) {
        const r = await pool.query("SELECT master_extended_cv FROM user_profiles WHERE id = $1", [state.userProfileId]);
...
async function directInterview(state: typeof StateAnnotation.State, config?: RunnableConfig) {
    if (state.missingCount === 0) {
        await dispatchCustomEvent("progress", { msg: "Profile complete. Bypassing interview." }, config);
    ...
    // History
    let historyText = state.interviewHistory?.map(h => `Q: ${h.question}\nA: ${h.answer}`).join("\n") || "No previous questions.";
    ...
    const updatedHistory = [...(state.interviewHistory || []), {
