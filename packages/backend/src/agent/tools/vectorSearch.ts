...
export const searchGithubProjectsTool = tool(
  async ({ query, limit = 5 }) => {
    try {
  ...
  },
  {
    ...
    description:
      "Search across all user GitHub repositories using semantic vector search. Use this to find specific code implementations, project purposes, and verify technical claims.",
    schema: z.object({
      query: z
        .string()
        .describe(
          "The semantic query or claim to search for (e.g. 'Used React with Redux for state management')"
        ),
...
export const querySkillsAndExperiencesTool = tool(
  async ({ userProfileId }) => {
    try {
      const result = await pool.query("SELECT demographics_json FROM user_profiles WHERE id = $1", [
        userProfileId,
      ]);
  ...
  },
  {
    ...
    description:
      "Fetch the JSON ontology containing all the user's verified skills, past work experiences, and educational background.",
    schema: z.object({
